...
class PredictionEngine {
    constructor(data) {
        ...
        // HOST BONUS REDUCIDO: +30 ELO (era +70, demasiado alto)
        // Justificación real: la ventaja de anfitrión en WC es ~0.3 goles extra,
        // que equivale a ~40 puntos ELO. Pero solo aplica en partidos de local
        // puros; en un Mundial neutro vale menos → usamos 30.
    ...
    // ============================================================
    // CÁLCULO DE PROBABILIDADES BASE (Modelo de Poisson)
    // ============================================================
    getGoalExpectancy(teamA, teamB) {
        // Venue Factor: Solo reciben bonus si son hostCountry y juegan contra alguien que no lo es
        ...
        const compositeEloA = (eloHistA * 0.40) + (eloSquadA * 0.60);
        const compositeEloB = (eloHistB * 0.40) + (eloSquadB * 0.60);
        ...
        let lambdaA = this.BASE_LAMBDA * Math.pow(pWinA / 0.5, 1.1);
        let lambdaB = this.BASE_LAMBDA * Math.pow(pWinB / 0.5, 1.1);

        // Form Bug Fix: Ajuste sutil por forma reciente
        lambdaA *= this.getFormMultiplier(teamA);
        lambdaB *= this.getFormMultiplier(teamB);

        // Ajuste por stats de equipo (ataque vs defensa)
        ...
        lambdaA = Math.max(0.3, Math.min(this.MAX_GOALS, lambdaA));
        lambdaB = Math.max(0.3, Math.min(this.MAX_GOALS, lambdaB));
    ...
    predictMatch(teamAName, teamBName) {
        ...
        return {
            ...
            winB:  parseFloat((winB  * 100).toFixed(1)),
        ...
        for (let i = 0; i < groupTeams.length; i++) {
            for (let j = i + 1; j < groupTeams.length; j++) {
                ...
                matches.push({
                    ...
                    goalsA: Math.round(lA), goalsB: Math.round(lB)
                });
            }
        }
